...
RUN cd /opt/rss-json-service && cargo install --locked trunk && rustup target add wasm32-unknown-unknown
...
RUN cd /opt/rss-json-service && cd /opt/podcast-player && trunk build --release
